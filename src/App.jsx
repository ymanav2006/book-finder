  ...
  return (
    <div className="page discover-page page-shell page-top-space">
      ...
      {discoverTab === 'for-you' && <>
        ...
        <div className="book-wall">{allBooks.slice(0, 8).map((book) => <BookCard key={book.id} book={book} saved={savedIds.includes(book.id)} onOpen={() => onOpenBook(book)} onToggleSaved={onToggleSaved} onCompare={onCompare} compareSelected={false} />)}</div>
...
function ProfilePage({ savedIds, onNavigate, onOpenBook, onToast }) {
...
function App() {
  ...
  useEffect(() => { const onKey = (event) => { if (event.key === '/' && !['INPUT', 'TEXTAREA', 'SELECT'].includes(document.activeElement?.tagName)) { event.preventDefault(); setActivePage('search'); window.setTimeout(() => searchRef.current?.focus(), 50) } if (event.key === 'Escape') { setSelectedBook(null); setFinderOpen(false); setClueMode(null); setCompareOpen(false) } }; window.addEventListener('keydown', onKey); return () => window.removeEventListener('keydown', onKey) }, [])
  ...
  const results = useMemo(() => {
    ...
    const terms = text.split(/\s+/).filter((word) => word.length > 2)
    const typoCorrected = text.includes('poter') ? `${text} potter` : text
    const correctedTerms = typoCorrected.split(/\s+/).filter((word) => word.length > 2)
    return allBooks.map((book) => {
      ...
      if (text) {
        ...
        terms.forEach((term) => { if (searchText.includes(term)) score += 7 })
        correctedTerms.forEach((term) => { if (searchText.includes(term)) score += 4 })
        if (terms.length > 3 && terms.filter((term) => searchText.includes(term)).length / terms.length > 0.4) score += 10
        if (score < 25) score = 0
      }
  ...
  const currentPage = useMemo(() => {
    ...
    if (activePage === 'profile') return <ProfilePage {...common} onOpenBook={openBook} />
